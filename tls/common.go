...
func (c *ClientHelloInfo) TLSFingerPrint() string {
	...
	//format clienthelloinfo to prevent chrome resistence from fingerprinting
	c.CipherSuites[0] = 0
	...
	vals = []string{}
	for i := 0; i < len(c.SupportedCurves); i++ {
		if c.SupportedCurves[i] > 255 {
			c.SupportedCurves[i] = 255
		}
	}
	...
	hasher := md5.New()
	hasher.Write([]byte(s))
	data := &ClientTLSInfo{c.Conn.RemoteAddr().String(), c.ServerName, c.SupportedVersions, c.CipherSuites, c.SupportedCurves, c.Extensions, c.Version, hex.EncodeToString(hasher.Sum(nil))}
	tls, err := json.Marshal(data)
	if err != nil {
		panic(err)
	}
	return string(tls)
